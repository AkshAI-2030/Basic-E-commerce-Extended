...
function productsSortPopularity(product1, product2) {
  return product2.rating - product1.rating;
}

app.get('/products/sort/popularity', (req, res) => {
  let productsCopy = products.slice();
  let sortedProducts = productsCopy.sort(productsSortPopularity);
  res.json({ products: sortedProducts });
});

function productsSortPricing(product1, product2) {
  return product2.price - product1.price;
}

app.get('/products/sort/price-high-to-low', (req, res) => {
  let productsCopy = products.slice();
  let sortedProducts = productsCopy.sort(productsSortPricing);
  res.json({ products: sortedProducts });
});

function productsSortPricingAsc(product1, product2) {
  return product1.price - product2.price;
}

app.get('/products/sort/price-low-to-high', (req, res) => {
  let productsCopy = products.slice();
  let sortedProducts = productsCopy.sort(productsSortPricingAsc);
  res.json({ products: sortedProducts });
});
...
app.get('/products/filter/ram', (req, res) => {
  ...
  let filterByRam = products.filter((product) =>
    productFilterByRam(product, ram)
  );
...
app.get('/products/filter/rom', (req, res) => {
  ...
  let filterByRom = products.filter((product) =>
    productFilterByRom(product, rom)
  );
...
app.get('/products/filter/os', (req, res) => {
  let os = req.query.os;
  let filterByOs = products.filter((product) => productFilterByOs(product, os));
